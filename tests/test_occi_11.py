...
def test_query_interface(url):
    ...
    if not response.status in [200]:
        msg += 'Failed to filter the query interface - ' + str(response.status) + ' ' + response.reason + ' ' + content
    if len(response['category'].split(',')) > 1:
        msg += 'The filter mechanism does not work - should only contain the compute category definition - but was: ' + str(response['category'])
    ...
    if not response.status in [200]:
        msg += 'Failed to add a user defined mixin - ' + str(response.status) + ' ' + response.reason + ' ' + content
...
def test_operations_on_mixins_or_kinds(url):
    ...
    for item in response1['x-occi-location'].split(', '):
        if item not in content.split('\n'):
            msg += 'X-OCCI-Location and uri-list do not return the same values for the compute collection...'
    ...
    if not response.status == 200:
        msg += 'Could not triggered action: ' + repr(response) + content
    ...
    if response.status != 200:
        msg += 'Unable to create a user-defined mixin. Response: ' + repr(response) + content

    post_heads = heads.copy()
    post_heads['X-OCCI-Location'] = compute_loc
    response, content = http.request(url + '/my_stuff/', 'POST', headers=post_heads)
    if response.status != 200:
        msg += 'Unable to add a kind to a user-defined mixin. Response: ' + repr(response) + content
    ...
    if response['category'].find('my_stuff') is - 1:
        msg += 'Mixin was not added to resource: ' + repr(response) + content
    ...
    if len(response['x-occi-location'].split(',')) == 0:
        msg += 'Collection seems to be empty: ' + repr(response) + content
    ...
    if response.status != 200:
        msg += 'The full update seems not to be working: ' + repr(response) + content

    response, content = http.request(url + '/my_stuff/', 'GET', headers=heads)
    if response['x-occi-location'] != compute_loc2:
        msg += 'Full update seems not to be working...' + repr(response) + content
    ...
    response, content = http.request(url + '/compute/', 'GET', headers=get_heads)
    if response['x-occi-location'] != compute_loc2:
        msg += 'Filtering based on categories seems not to be working...' + repr(response) + content
    ...
    if len(response['x-occi-location'].split()) != 1:
        msg += 'Filtering based on attributes seems not to be working...' + repr(response) + content
    ...
    if response.status != 200:
        msg += 'Unable to remove last entity from mixin. Response: ' + repr(response) + content
        exit(0)

    response, content = http.request(url + '/my_stuff/', 'GET', headers=heads)
    if hasattr(response, 'x-occi-location'):
        msg += 'The collection should be empty but server responded: ' + repr(response) + content
    ...
    if response.status != 200:
        raise AttributeError('Unable to delete a compute instances. Response: ' + repr(response) + content)
...
def test_operations_on_resource_instances(url):
    ...
    if response.status == 201:
        ...
        msg += 'Could not create compute kind...'
    ...
    if not response.status == 200:
        msg += 'Unable to do an update on the resource: ' + loc
    ...
    if not response.status in [200, 201]:
        msg += 'Failed to put a new compute resource at /123/. response: ' + repr(response) + content
    ...
    response, content = http.request(url + '/123', 'PUT', headers=put_heads)
    if not response.status == 200:
        msg += 'Failed to do a full update on compute resource at /123/. response: ' + repr(response) + content

    # GET
    response, content = http.request(loc, 'GET', headers=heads)
    if not response.status == 200:
        msg += 'Unable to do retrieve the resource: ' + loc

    # DELETE
    response, content = http.request(loc, 'DELETE', headers=heads)
    if not response.status == 200:
        msg += 'Unable to do delete the resource: ' + loc
...
def test_handling_link_instances(url):
    ...
    if response.status not in [200, 201]:
        msg += 'Creation failed -  please examine output! ' + repr(response) + content
        logging.error(msg)
    ...
    if response.has_key('x-occi-attribute'):
        if response['x-occi-attribute'].find('occi.core.source') == -1 or response['x-occi-attribute'].find('occi.core.target') == -1:
            msg += 'The link seems to be missing source an target attributes. Response' + repr(response) + content
    else:
        msg += 'Something did go wrong during the GET on the link. Response: ' + repr(response) + content

    # 1st cleanup...
    response, content = http.request(compute_loc, 'DELETE', headers=heads)
    if not response.status == 200:
        msg += 'Unable to do delete the resource: ' + compute_loc
    ...
    if not response.has_key('link'):
        msg += 'Inline link rendering for the compute resource seems to be missing. Response: ' + repr(response) + content
    ...
    if not response.status == 200:
        msg += 'Unable to do delete the resource: ' + network_loc
...
def test_syntax(url):
    ...
    m = p.match(cat_rend)
    if m is None:
        msg += 'There is an error in the syntax for rendering text/plain. Category should be setup like <term>;scheme="<url>";class=[kind,action,mixin]'
    ...
    m = p.match(cat_rend)
    if m is None:
        msg += 'There is an error in the syntax for rendering text/occi. Category should be setup like <term>;scheme="<url>";class=[kind,action,mixin]'
...
def test_content_type_and_accept_headers(url):
    ...
    if not response['content-type'] == 'text/plain':
        msg += 'When requesting text/plain - The Content-type text/plain should be exposed by the server.'

    heads = {'Accept': 'text/occi'}
    response, content = http.request(url, 'GET', headers=heads)
    if not response['content-type'] == 'text/occi':
        msg += 'When requesting text/occi - The Content-type text/occi should be exposed by the server.'
...
def test_rfc5785_compliance(url):
    ...
    if not response1.status in [200]:
        msg += 'Get on Query Interface failed - ' + str(response1.status) + ' ' + response1.reason + ' ' + content1

    # retrieval of all kinds, actions and mixins
    response2, content2 = http.request(url + '/-/', 'GET', headers=heads)
    if not response2.status in [200]:
        msg += 'Get on Query Interface failed - ' + str(response2.status) + ' ' + response2.reason + ' ' + content2

    if response1['category'] != response2['category']:
        msg += 'Reponse differ for /-/ and .well-known/...'
...
def test_occi_infrastructure(url):
    ...
    if not 'compute' in infra_model:
        msg += 'Missing compute kind.'
    if not 'network' in infra_model:
        msg += 'Missing network kind.'
    if not 'storage' in infra_model:
        msg += 'Missing storage kind.'
    if not 'storagelink' in infra_model:
        msg += 'Missing storagelink link.'
    if not 'networkinterface' in infra_model:
        msg += 'Missing networkinterface link.'
    if not 'ipnetwork' in infra_model:
        msg += 'Missing ipnetwork mixin.'
    if not 'start' in infra_model:
        msg += 'Missing start action definition.'
    if not 'stop' in infra_model:
        msg += 'Missing stop action definition.'
    if not 'restart' in infra_model:
        msg += 'Missing restart action definition.'
    if not 'suspend' in infra_model:
        msg += 'Missing suspend action definition.'
    if not 'up' in infra_model:
        msg += 'Missing up action definition.'
    if not 'down' in infra_model:
        msg += 'Missing down action definition.'
    if not 'snapshot' in infra_model:
        msg += 'Missing snapshot action definition.'
    if not 'resize' in infra_model:
        msg += 'Missing resize action definition.'
    if not 'online' in infra_model:
        msg += 'Missing online action definition.'
    if not 'offline' in infra_model:
        msg += 'Missing offline action definition.'
    if not 'backup' in infra_model:
        msg += 'Missing backup action definition.'
